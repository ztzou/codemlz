      ...
      */
      if(com.NSsites && com.model==0) {
         for(j=0,dS=dN=0; j<com.ncatG; j++) {
            ...
            mr = -1;
            eigenQcodon(2,1,&S,&dSt,&dNt,NULL,NULL,NULL, &mr, com.pkappa,com.rK[j],com.eta, PMat);
      ...
      if(!com.fix_omega && com.NSsites==0 && com.model==0 && com.aaDist!=7 && com.Mgene<=2)
         fprintf(fout,"\nomega (dN/dS) = %8.5f\n", x[k++]);
      if(!com.fix_eta && com.NSsites==0 && com.model==0 && com.aaDist==0 && com.Mgene==0)
    	 fprintf(fout,"\neta (dIN/dVN) = %8.5f\n", x[k++]);
...
/*       if(com.codonf >= FMutSel0)
  ...
  */
         ...
         if(com.NSsites==0) {
            ...
            mr = 0;
            eigenQcodon(2,t,&S,&dS,&dN, NULL,NULL,NULL, &mr, com.pkappa,com.eta,om,PMat); /* PMat destroyed! */
            dNt += dN;
            dSt += dS;
...
int GetOptions (char *ctlf)
{
   ...
   char *optstr[] = {"seqfile", "outfile", "treefile", "seqtype", "noisy",
        ...
        "fix_rho", "rho", "ndata", "bootstrap", "Small_Diff", "fix_blength","fix_eta","eta"};
   double t;
   FILE  *fctl;
      ...
      for (iopt=0; iopt<nopt; iopt++) {
         if (strncmp(opt, optstr[iopt], 8)==0)  {
            ...
            switch (iopt) {
               ...
               case (37): com.fix_eta=(int)t;     break;
               case (38): com.eta=t;              break;
           }
           break;
         }
      }
...
*/
         ...
         if(com.aaDist==0) {
            if((!com.fix_omega || (com.Mgene && com.Mgene>=3)) && !com.NSsites && !com.model)
               com.nrate++; /* Include a single omega in nrate */
         }
         else {
         ...
         } /* Finish setting com.nrate for omega(s). (Ignoring site models) */

         /* Now add eta to com.nrate */
         com.nrate += !com.fix_eta;
...
int SetxBound (int np, double xb[][2])
{
   ...
   double alphab[]={0.02,49}, betab[]={0.005,99}, omegab[]={0.0001,999}, etab[]={0.001,999};
   ...
   if (com.aaDist<0 && (com.seqtype==1||com.model==FromCodon)) {
      ...
      printf("\nEntered, com.ntime+com.nrgene+!com.fix_kappa = \n", com.ntime+com.nrgene+!com.fix_kappa);
   }

   if(com.seqtype==CODONseq && !com.model && !com.aaDist && !com.NSsites && !com.fix_eta) {
	   //printf("\nk before setting bound for eta = %d\n",k);
	   k++; xb[k][0]=etab[0]; xb[k][1]=etab[1];
   }

   //printf("\nk after setting bound for eta = %d\n",k);

   k=com.ntime+com.nrgene+com.nrate;

   //printf("\nk=com.ntime+com.nrgene+com.nrate = %d\n", k);
...
*/
   ...
   if(com.nrate) { /* either kappa, omega, eta, or any combinations for each gene */
      if(com.Mgene<=2) {
         if(com.hkyREV) { /* False */
            x[k++]=.5+rndu();
...
*/
   ...
   } /* if(com.model && com.model<=NSbranch3) */


   if (com.NSsites==0 && com.nrate==0)
      eigenQcodon(1,-1,NULL,NULL,NULL,Root,U,V, &mr, (com.nkappa>1?x+com.ntime+com.nrgene:&com.kappa), com.omega,com.eta,PMat);
...
{
/* This calculates the number of parameters (com.np) and get initial values.
...
*/
   ...
   GetInitialsTimes(x); /* set initial branch lengths: com.ntime branches */
   //printf("\nFinish GetInitialTimes, x[:3] = %f %f %f\n",x[0],x[1],x[2]);
   ...
   *fromfile=0;
   if(finitials) { /* Not sure what this do */
      readx(x,fromfile);
      ...
      if (com.seqtype==CODONseq) {
         ...
         if (!com.NSsites)
            eigenQcodon(1,-1,NULL,NULL,NULL,Root,U,V, &mr,
               (com.hkyREV||com.codonf==FMutSel?&xcom[com.nrgene]:&com.kappa),com.omega,com.eta,PMat);
      }
      else
   ...
   */
   ...
   if(com.model == 0) {  /* NSsites models */
      if(com.aaDist==0) {
         ...
         Qfactor_NS = -1;
         eigenQcodon(0,-1,&S,&dS,&dN,NULL,NULL,NULL, &Qfactor_NS, com.pkappa, mr, com.eta, PMat);
      }
      else {
         for(j=0,Qfactor_NS=0; j<com.ncatG; j++) {
            ...
            mr = -1;
            eigenQcodon(0,-1,&S,&dS,&dN,NULL,NULL,NULL, &mr, com.pkappa, com.rK[j], com.eta, PMat);
      ...
      for(i=0; i<2; i++) {  /* i=0 back (2 site classes); i=1 fore (3 classes) */
         ...
         eigenQcodon(0,-1,&S,&dS,&dN,NULL,NULL,NULL, &Qfactor_NS_branch[i], com.pkappa, mr, com.eta, PMat);
      ...
      */
      for(i=0; i<3; i++) {  /* (w0,w1,w2) */
         ...
         mr = 1;
         eigenQcodon(1,-1,NULL,NULL,NULL,_Root[i],_UU[i],_VV[i], &mr, com.pkappa,w[1][i],com.eta,PMat);
      }
   }
   else { /* NSbranch3: Clade models C and D */
      ...
      for(i=0; i<com.nbtype; i++) {
         ...
         Qfactor_NS_branch[i] = -1;
         eigenQcodon(0,-1,NULL,NULL,NULL,NULL,NULL,NULL, &Qfactor_NS_branch[i], com.pkappa,mr,com.eta,PMat);
         Qfactor_NS_branch[i] = 1/Qfactor_NS_branch[i];
      ...
      */
      for(i=0; i<K-1+com.nbtype; i++) {
         ...
         eigenQcodon(1,-1,NULL,NULL,NULL,_Root[i],_UU[i],_VV[i], &mr, com.pkappa,com.omega,com.eta,PMat);
      }
   }
   return(0);
}
...
*/
   ...
   if(com.seqtype==1 || com.model==FromCodon || com.aaDist==AAClasses) {
      k = com.ntime+com.nrgene;
      //printf("\nSetParameters(): k = %d, com.nrgene = %d\n",k,com.nrgene);
      if(com.hkyREV==0) { /* this block is entered */
         if(com.fix_kappa==1) { com.pkappa[0]=com.kappa; ik=1; }
         else                   com.kappa=x[k]; /* Set kappa */
      }
      for(i=0; i<com.nkappa; i++)
         com.pkappa[ik++] = x[k++]; /* Set kappas */

      if(com.npi) {
         com.ppi = x+com.ntime+com.nrgene+com.nkappa; /* Set pointer for storing pi */
         GetCodonFreqs ();
      }
      com.pomega = x+com.ntime+com.nrgene+com.nkappa+com.npi; /* Set pointer for storing omega */
   }
   for(j=0;j<com.nrgene;j++)
      com.rgene[j+1] = x[com.ntime+j];
   if(com.clock && AbsoluteRate) com.rgene[0] = x[0]; /* so that rgene are abs rates */

   if(com.seqtype==1 && com.aaDist>=FIT1) /* ignore */
      getpcodonClass(x, pcodonClass);

   k=com.ntime+com.nrgene+com.nkappa+com.npi;

   //printf("\nSetParameters(): k = %d, com.eta = %d\n",k,com.eta);
   if (com.nrate) {
      if(!com.model && !com.aaDist && !com.NSsites){
    	 if(!com.fix_omega) {
            com.omega=x[k++];
    	 }
         if(!com.fix_eta) {
        	 com.eta=x[k];com.peta = &x[k];
         }
      }
      if(com.seqtype==AAseq)
         eigenQaa(NULL, Root, U, V, x+com.ntime+com.nrgene);
    	  ...
    	  /* added parameter eta */
         eigenQcodon(1,-1,NULL,NULL,NULL,Root,U,V, &mr, com.pkappa, com.omega,com.eta,PMat);
      else if((com.model==NSbranchB || com.model==NSbranch2)
         ...
         for(i=0; i<com.nbtype; i++) {
            ...
            eigenQcodon(1,-1,NULL,NULL,NULL,_Root[i],_UU[i],_VV[i], &mr, com.pkappa,w0,com.eta,PMat);
         }
      }
      k = com.ntime+com.nrgene+com.nrate;
   }
   ...
   if(mode==1) {  /* get Root, U, & V */
      ...
      for (i=0; i<n; i++) {
         ...
         for(j=0; j<n; j++) {
            ...
            else if(from[pos]+to[pos]==1 || from[pos]+to[pos]==5){
               ra += piQij;
...
*/
   ...
   case (FromCodon): /* eigenQcodon check mode value */
      eigenQcodon(0,-1,NULL,NULL,NULL,Root,U,V, &mr,
         (com.hkyREV||com.codonf==FMutSel?rate:&com.kappa),com.omega,com.eta,Qc);
...
*/
   ...
   */
   ...
   for(ig=0; ig<com.ngene+1; ig++) {
      ppi = (ig<com.ngene?com.piG[ig]:com.pi); /* pi pointer */
      zero(ppi, nc);
...
*/
   ...
   /* printf("\nt & w: %.5f %.5f\n", t, omega);
      matout(F0,vtw, 2,2); */
   eigenQcodon(2,t,&S,&dS,&dN,NULL,NULL,NULL, &mr, com.pkappa,omega,com.eta,PMat);

   eh = (t+1)*Small_Diff;
   eigenQcodon(2,t+eh,&S,&dS1,&dN1,NULL,NULL,NULL, &mr, com.pkappa,omega,com.eta,PMat);
   eigenQcodon(2,t-eh,&S,&dS2,&dN2,NULL,NULL,NULL, &mr, com.pkappa,omega,com.eta,PMat);
   JacobiSN[0*np+0] = (dS1 - dS2)/(2*eh);
   JacobiSN[1*np+0] = (dN1 - dN2)/(2*eh);

   eh = (omega+1)*Small_Diff;
   eigenQcodon(2,t,&S,&dS1,&dN1,NULL,NULL,NULL, &mr, com.pkappa,omega+eh,com.eta,PMat);
   eigenQcodon(2,t,&S,&dS2,&dN2,NULL,NULL,NULL, &mr, com.pkappa,omega-eh,com.eta,PMat);
...
*/
   ...
   for(is=0;is<com.ns;is++) {
      ...
      for(js=0; js<is; js++) {
         ...
         FPN(fout);
         eigenQcodon(2,x[0],&S,&dS,&dN, NULL,NULL,NULL, &mr, com.pkappa,com.omega,com.eta,PMat);
         ...
         if(com.getSE && !com.fix_omega) {
            FOR(k, np) {
               ...
               eigenQcodon(2,y[0],&S,&dS1,&dN1,NULL,NULL,NULL, &mr, com.pkappa,com.omega,com.eta,PMat);
               y[k] -= 2*eh;
               if(!com.fix_kappa) com.kappa = y[1];
               com.omega = y[1+!com.fix_kappa];
               eigenQcodon(2,y[0],&S,&dS2,&dN2,NULL,NULL,NULL, &mr, com.pkappa,com.omega,com.eta,PMat);
...
*/
  ...
  if( ( pS>0 && pS<0.74 ) && ( pN>0 && pN<0.74 ) && x[0] > 0.001 && x[0] < 10 && x[2] > 0.005 && x[2] < 5 ){
   ...
   for(i = 0; i < npoints; i++) {  //count for omega
       ...
       eigenQcodon( 1, -1, NULL, NULL, NULL, Root, U, V, &scalefactor, kappa, w_value, com.eta, Qmatrix );  //estimate Root,U,V for the scaled Qmatrix

	   //For P(w>1|x) calculation
	   if(setp == 0) {
	      u = ( (1 - alpha) * z2 + 1 + alpha ) / 2;
	      w_p = logistic_transformation( u, m2, s2 );
   	   eigenQcodon(1, -1, NULL, NULL, NULL, Rp, Up, Vp, &scalefactor, kappa, w_p, com.eta, Qmatrix );  //estimate Rp,Up,Vp for the scaled Qmatrix for the calculation of P(w>1|x)
      }
...
*/
   ...
   com.ncode=n; com.cleandata=1; com.eta=1.; setmark_61_64 ();
   for(j=0; j<com.ns; j++)
   ...
   for(ip=0; ip<99; ip++) {
      ...
      for(j=0,Qfactor_NS=0,dS=dN=0; j<com.ncatG; j++) {
         ...
         eigenQcodon(2,1,&S,&dSt,&dNt,NULL,NULL,NULL, &mr, &kappa0,com.rK[j],com.eta,PMat);
         dS += freqK_NS*dSt;
         dN += freqK_NS*dNt;
      }
      Qfactor_NS = 1/Qfactor_NS;
      ...
      eigenQcodon(2,t0,&S,&dS,&dN, NULL,NULL,NULL, &mr, &kappa0,omega0,com.eta,space);
      om=omega0;
#endif
      printf("\nCorrect values");
      ...
      for(k=0,zero(Efij,npatt0); k<com.ncatG; k++) {
         eigenQcodon(1,-1,NULL,NULL,NULL,Root,U,V, &mr, &kappa0,com.rK[k],com.eta,PMat);
      ...
      com.NSsites=0;
#else
      eigenQcodon(1,-1,NULL,NULL,NULL,Root, U, V, &mr, &kappa0, omega0,com.eta, PMat);
      PMatUVRoot (PMat, t0, n, U, V, Root);
      ...
      for(il=0; il<nil+1; il++) {
         ...
         if(com.ls==0) {
            ...
            printf("lnL = %.6f\n",-lnL);
            eigenQcodon(2,x[0],&S,&dS,&dN, NULL,NULL,NULL, &mr, &x[1],x[2],com.eta,space);
         ...
         for(ir=0,zero(mx,6),zero(vx,6),zero(mse,6); ir<nr; ir++) {
            ...
            eigenQcodon(2,x[0],&S,&x[3],&x[4], NULL,NULL,NULL, &mr, &x[1],x[2],com.eta,space);
            FOR(j,5) {
...
printf("\tt\tS\tdS\tdN\tdN/dS\tS4\td4\tk_4f\tpT_4f\n");
   ...
   eigenQcodon(1,-1,NULL,NULL,NULL,Root,U,V, &mr, &com.kappa,com.omega,com.eta,PMat);

matout(frst,com.pi,16,4);

   FOR(ii,nii) {
      t=t0[ii];
      eigenQcodon(2,t,&S,&dS,&dN,NULL,NULL,NULL, &mr, &com.kappa,com.omega,com.eta,PMat);
